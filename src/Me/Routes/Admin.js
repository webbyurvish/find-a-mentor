...
const pending = (status) => status === STATUS.new || status === STATUS.viewed;

const UserDetails = ({ userId, mentorships }) => {
  const [isLoading, setIsLoading] = useState(true);
  const [dontActiveSent, setDontActiveSent] = useState();
...
const Admin = () => {
  ...
  const [user, setUser] = useState();
  const [mentorshipLoading, setMentorshipLoading] = useState();
          ...
          return (
            <tr style={{ opacity: mentor.available ? 1 : 0.5 }} key={id}>
              <td>
                ...
                <a target="_blank" rel="noreferrer" href={`/u/${mentor.id}`}>
                  🔗
                </a>
              </td>
              <td>
                <Mentee onClick={() => setUser(mentee)}>
                  ...
                  <a target="_blank" rel="noreferrer" href={`/u/${mentee.id}`}>
                    @
                  </a>
                </Mentee>
              </td>
