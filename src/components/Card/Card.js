...
const tagsList = (tags, handleTagClick, showAll = false) => {
  return (
    <>
      {tags
...
const CTAButton = ({ tooltipProps, onClick, text, link }) => {
  if (!onClick && !link) {
    // eslint-disable-next-line no-console
    console.warn('CTA button must have either onClick or link');
  }
  const CTAElement = ({ children }) =>
    link ? (
...
const Card = ({ mentor, onFavMentor, isFav, appearance }) => {
