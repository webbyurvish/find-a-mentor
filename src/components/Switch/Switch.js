...
let SwitchInputRoot = styled.div`
  height: 20px;
...
const SwitchLabel = ({ id, label }) => (
  <label id={id} htmlFor={`switch-input-${id}`}>
    {label}
  </label>
);

const SwitchInput = ({ onToggle, isChecked, id, size = 'normal' }) => {
...
const Switch = ({ id: idFromProps = '', label, onToggle, size, isChecked }) => {
